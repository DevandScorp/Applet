...
public class Main extends Applet {
    private int WIDTH;
    private int HEIGHT;
    private final int DRAW_STEP = 40;
    private final double COORD_STEP = 0.05;
    @Override
    public void init() {
    ...
    public void paint(Graphics g) {
        ...
        for (int i = 0, count = -5; i < WIDTH; i += DRAW_STEP, ++count) {
        ...
        for (int i = 0; i < 60; ++i, step += COORD_STEP) {
            xCoord[i] = -1 + step;
        ...
        int[] y = new int[60];
        for (int i = 0; i < 60; ++i) {
            x[i] = HEIGHT / 2 + (int) (xCoord[i] * DRAW_STEP);
            y[i] = WIDTH / 2 - (int) (yCoord[i] * DRAW_STEP);
        }
